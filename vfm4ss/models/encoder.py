...
class Encoder(nn.Module):
    def __init__(
        ...
        data_config = resolve_model_data_config(self.encoder)

        pixel_mean = torch.tensor(data_config["mean"]).reshape(1, -1, 1, 1)
        pixel_std = torch.tensor(data_config["std"]).reshape(1, -1, 1, 1)

        self.register_buffer("pixel_mean", pixel_mean)
        self.register_buffer("pixel_std", pixel_std)

        self.grid_size = tuple(round(size / patch_size) for size in img_size)
        self.feature_grid_size = self.grid_size
    ...
    def forward(self, x: torch.Tensor):
        ...
        if x.dim() == 4:
            self.feature_grid_size = tuple(int(size) for size in x.shape[-2:])
            x = x.flatten(2).transpose(1, 2)
        else:
            x = x[:, self.encoder.num_prefix_tokens :]
            self.feature_grid_size = self._resolve_feature_grid_size(x.shape[1])

        return x

    def _resolve_feature_grid_size(self, num_tokens: int) -> tuple[int, int]:
        ...
        if self.grid_size[0] * self.grid_size[1] == num_tokens:
            return self.grid_size

        grid_h = int(math.isqrt(num_tokens))
        while grid_h > 1 and num_tokens % grid_h != 0:
            grid_h -= 1

        grid_w = num_tokens // grid_h
        if grid_h * grid_w != num_tokens:
            raise ValueError(f"Could not infer feature grid size from {num_tokens} tokens.")
